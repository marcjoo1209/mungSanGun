					...
					<tbody>
						<c:forEach items="${outDto}" var="list" varStatus="status">
							<tr data-toggle="modal" data-target="#modifyModal"
								onclick="clickOrderItemView(${list.IDX})">
								<td>${list.IDX}</td>
								<td>${list.TARGETCOMMANT}</td>
								<td>
								...
								<td>${list.TARGETWEBURL}</td>
								<td>${list.TARGETAMT}</td>
								<td>
									<c:forEach items ='${codeOutDto }' var ='list1' varStatus='status'>
										<c:if test='${list1.CODEID eq "GBUPDOWN"}'>
											<c:if test='${list1.CODEVAL1 eq list.GBUPDOWN}'>
												${list1.CODEVAL2}
											</c:if>
										</c:if>
									</c:forEach>
								...
								<td>
									...
									<c:forEach items ='${codeOutDto }' var ='list1' varStatus='status'>
										<c:if test='${list1.CODEID eq "GBCARD"}'>
											<c:if test='${list1.CODEVAL1 eq list.GBCARD}'>
												${list1.CODEVAL2}
											</c:if>
										</c:if>
									</c:forEach>
								</td>
								<td>${list.ALAMSOLDOUT}</td>
								<td>${list.ID}</td>
								<td>${list.PWD}</td>
								<td>${list.ALAMYN}</td>
								<td>
									<button type="button" class="btn btn-primary" id="removeButton" onclick="removeButton_onclikc(${list.IDX})" style="font-size: 10px">삭제</button>
								</td>
							</tr>
						</c:forEach>
					</tbody>
			...
			success : function(obj){
				...
				var json = JSON.parse(data);
			  	...
			  	let htmlTxt = 
			        ...
			        +    "<label for='recipient-name0' class='col-form-label'>순서:</label>"
			        +	"<input type='text' class='form-control' id='recipient-name0' value='"+json.rst0+"' readonly>"
			        + "</div>"

			  		+ "<div class='form-group'>"
			        +    "<label for='recipient-name1' class='col-form-label'>대상웹페이지별명:</label>"
			        +	"<input type='text' class='form-control' id='recipient-name1' value="+json.rst1+">"
			        + "</div>"

			  		+ "<div class='form-group'>"
			        +    "<label for='recipient-name2' class='col-form-label'>이커머스순서:</label>"
			        //+	"<input type='text' class='form-control' id='recipient-name2' value="+json.rst2+">"
			        ...
			        + " 			<c:if test='${list.IDX eq \"" + json.rst2 + "\"}'>"
			        + "					selected"
			        + "				</c:if>>"
			        + "				${list.SHOPNM}"
			        + "		</option>"
			        ...
			        + "</div>"
			  		...
			  		+ "<div class='form-group'>"
			        +    "<label for='recipient-name3' class='col-form-label'>대상웹페이지주소:</label>"
			        +	"<input type='text' class='form-control' id='recipient-name3' value="+json.rst3+">"
			        + "</div>"

			  		+ "<div class='form-group'>"
			        +    "<label for='recipient-name4' class='col-form-label'>기준 금액:</label>"
			        +	"<input type='text' class='form-control' id='recipient-name4' value="+json.rst4+">"
			        + "</div>"

			  		+ "<div class='form-group'>"
			        +    "<label for='recipient-name5' class='col-form-label'>0:최소, 1:최대 초과 구분:</label>"
			        //+	"<input type='text' class='form-control' id='recipient-name5' value="+json.rst5+">"
			        
			        + "<select class='form-control' id='recipient-name5'>"
			        + "  	<option value=''>--- 기준 선택 ---</option>"
			        + "			  	<c:forEach items ='${codeOutDto }' var ='list' varStatus='status'>"
			        + " 				<c:if test='${list.CODEID eq \"GBUPDOWN\"}'>"
			        + "		<option value='${list.CODEVAL1}' <c:if test='${list.CODEVAL1 eq \""+json.rst5+"\"}'>selected</c:if>>${list.CODEVAL2}</option>"		
			        + "					</c:if>"
			        + "			  	</c:forEach>"
			        + "</select>"
			        ...
			        + "</div>"
			  		...
			  		+ "<div class='form-group'>"
			        +    "<label for='recipient-name6' class='col-form-label'>0:쿠폰, 1:카드 구분:</label>"
			        //+	"<input type='text' class='form-control' id='recipient-name6' value="+json.rst6+">"
			        
			        + "<select class='form-control' id='recipient-name6'>"
			        + "  	<option value=''>--- 쿠폰, 카드 선택 ---</option>"
			        + "			  	<c:forEach items ='${codeOutDto }' var ='list' varStatus='status'>"
			        + " 				<c:if test='${list.CODEID eq \"GBCARD\"}'>"
			        + "		<option value='${list.CODEVAL1}' <c:if test='${list.CODEVAL1 eq \""+json.rst6+"\"}'>selected</c:if>>${list.CODEVAL2}</option>"		
			        + "					</c:if>"
			        + "			  	</c:forEach>"
			        + "</select>"
			        ...
			        + "</div>"
			  		...
			  		+ "<div class='form-group'>"
			        +    "<label for='recipient-name7' class='col-form-label'>품절시 알람 요청 Y:알람요청:,N:알람미요청:</label>"
			        // +	"<input type='text' class='form-control' id='recipient-name7' value="+json.rst7+">"
			        
			        + "<select class='form-control' id='recipient-name7'>"
			        + "		<option value='Y' <c:if test='${"+json.rst7+" eq \"Y\"}'> selected </c:if>>Y</option>"
			        + "		<option value='N' <c:if test='${"+json.rst7+" eq \"N\"}'> selected </c:if>>N</option>"		
			        + "</select>"
	        	        
			        + "</div>"

			  		+ "<div class='form-group'>"
			        +    "<label for='recipient-name8' class='col-form-label'>로그인id:</label>"
			        +	"<input type='text' class='form-control' id='recipient-name8' value="+json.rst8+">"
			        + "</div>"

			  		+ "<div class='form-group'>"
			        +    "<label for='recipient-name9' class='col-form-label'>로그인pwd:</label>"
			        +	"<input type='text' class='form-control' id='recipient-name9' value="+json.rst9+">"
			        + "</div>"
	 	   			+ "<tr>"

			  		+ "<div class='form-group'>"
			        +    "<label for='recipient-name10' class='col-form-label'>생성일자:</label>"
			        +	"<input type='text' class='form-control' id='recipient-name10' value='"+json.rst10+"' readonly>"
			        + "</div>"

			  		+ "<div class='form-group'>"
			        +    "<label for='recipient-name11' class='col-form-label'>수정일자:</label>"
			        +	"<input type='text' class='form-control' id='recipient-name11' value='"+json.rst11+"' readonly>"
			        + "</div>"
	 	   			+ "<tr>"

			  		+ "<div class='form-group'>"
			        +    "<label for='recipient-name12' class='col-form-label'>알람 여부:</label>"
			        //+	"<input type='text' class='form-control' id='recipient-name12' value="+json.rst12+">"
			        
			        + "<select class='form-control' id='recipient-name12'>"
			        + "		<option value='Y' <c:if test='${"+json.rst12+" eq \"Y\"}'> selected </c:if>>Y</option>"
			        + "		<option value='N' <c:if test='${"+json.rst12+" eq \"N\"}'> selected </c:if>>N</option>"		
			        + "</select>"
			        ...
			        + "</div>"
			  		...
			  		+ "<div class='form-group'>"
			        +    "<label for='recipient-name13' class='col-form-label'>수정 사용자:</label>"
			        +	"<input type='text' class='form-control' id='recipient-name13' value="+json.rst13+">"
			        + "</div>"
	 	   			+ "<tr>"
	 	   			;
