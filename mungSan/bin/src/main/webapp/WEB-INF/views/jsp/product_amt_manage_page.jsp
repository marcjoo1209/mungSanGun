...
<div class="row">
					...
					<tbody>
						<c:forEach items="${outDto}" var="list" varStatus="status">
							<tr data-toggle="modal" data-target="#modifyModal"
								onclick="clickOrderItemView(${list.IDX})" align='center'>
								<td>${list.IDX}</td>
								...
								<td>
									<a href='${list.PRODUCTURL}'>링크</a>
								</td>
								<td>${list.LOGINID}</td>
								<td>${list.LOGINPWD}</td>
								<td>
									<c:forEach items ='${codeOutDto }' var ='list1' varStatus='status'>
										<c:if test='${list1.CODEID eq "GBAMT"}'>
											<c:if test='${list1.CODEVAL1 eq list.AMTGB}'>
												${list1.CODEVAL2}
											</c:if>
										</c:if>
									</c:forEach>
	...
	<form id="modifyProductSearchLst" method="post" action="/modifyProductSearchLst">
